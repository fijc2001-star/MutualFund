...
> **Backtest vs. Forward-test vs. Graficación — la división limpia (evitar confusión):**
> - **Backtest (histórico):** solo el motor en `core` de Python. Herramienta privada de I+D / de diseño del diseñador. Fácil de sobreajustar, por lo que **no** es la base del historial del mercado.
> - **Forward-test (en vivo, hacia adelante):** el **sandbox por suscripción durante `Evaluation`/`Listed`** — ejecutado por *nuestro* motor, por lo que es **verificable y alimenta el ledger resistente a manipulaciones + la puerta de calificación** (§5.8.1, §1.2.1). **Este rendimiento forward/en vivo es lo que vendemos**, porque los resultados forward no se pueden seleccionar sesgadamente como los backtests.
> - **Graficación:** **TradingView Lightweight Charts** (la librería, renderizada *dentro de nuestra app* con nuestros propios datos) es la superficie de visualización para señales y apertura/cierre de posiciones — *no* TradingView.com y *no* Pine Script. La venta ocurre vía el mercado + Stripe, nunca vía TradingView.
